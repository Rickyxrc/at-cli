...
def fetch_result(console:Console, contest_id:str, sids:list):
    with Progress(console=console) as progress:
        ...
        while not progress.finished:
            ...
            for sid in sids:
                status_string = res.json()['Result'][str(sid)]['Html']
                if "WJ" in status_string:
                    pass
                elif "waiting-judge" in status_string:
                    verdict, now, total, _ = re.findall(r'title="(.+)">(\d+)/(\d+)\s*(\S*)</span', status_string)[0]
                    ...
                    status_string = res.json()['Result'][str(sid)]['Html']
                    verdict, _ = re.findall(r'title="(.+)">(\S*)</span', status_string)[0]
...
def handle(console:Console):
    ...
    contest_id = console.input("input contest_id:")
    sid = console.input("input sid:")
    fetch_result(console, contest_id, [int(sid),])
