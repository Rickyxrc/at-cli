...
def confirm(console: Console, text:str, default:str = "type default selection here(yes or no), not required"):
    inputyes = ["y", "yes"]
    inputno = ["n", "no"]
    ...
    while True:
        ...
        if choice == '':
            if default.lower() in inputyes: return True
            if default.lower() in inputno: return False
            console.print(r'this input have no default operation.')
        if choice.lower() in inputyes: return True
        if choice.lower() in inputno: return False
